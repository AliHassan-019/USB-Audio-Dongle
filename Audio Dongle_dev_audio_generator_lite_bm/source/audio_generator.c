...
// -----------------------------------------------------------------------------
 // 1) AUDIO PACKET & RING-BUFFER SIZES (Full-Speed only)
// -----------------------------------------------------------------------------
#define AUDIO_PKT_SZ      FS_ISO_IN_ENDP_PACKET_SIZE
#define AUDIO_RING_BUF_SZ (AUDIO_PKT_SZ * 8)

static uint8_t micRing[AUDIO_RING_BUF_SZ];
static volatile uint32_t micWr = 0, micRd = 0;

static uint8_t spkRing[AUDIO_RING_BUF_SZ];
static volatile uint32_t spkWr = 0, spkRd = 0;
...
// --- USB Audio DMA-aligned buffers and state struct (NXP example) ---
USB_DMA_INIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_wavBuff[AUDIO_PKT_SZ];        // microphone → host

static uint8_t spkUsbBuf[AUDIO_PKT_SZ];        // host → speaker

USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
static uint8_t s_SetupOutBuffer[8];            // control-endpoint staging buffer
...
usb_audio_generator_struct_t s_audioGenerator = {
    ...
    .currentInterfaceAlternateSetting       = {0, 0},
...
// 2) Called from your DMIC/PDM ISR when each DMA buffer is ready:
// -----------------------------------------------------------------------------
void AUDIO_PushMicData(const uint8_t *data, uint32_t len)
{
    for (uint32_t i = 0; i < len; i++)
    {
        micRing[micWr] = data[i];
        micWr = (micWr + 1) % AUDIO_RING_BUF_SZ;
        if (micWr == micRd)
            micRd = (micRd + 1) % AUDIO_RING_BUF_SZ; // prevent overflow
    }
}

// Microphone IN transfer callback: called when host requests mic data
void USB_AudioRecorderGetBuffer(uint8_t *buffer, uint32_t size)
{
    uint32_t avail = (micWr + AUDIO_RING_BUF_SZ - micRd) % AUDIO_RING_BUF_SZ;
    if (avail < size) {
        // underflow → send zeros
        memset(buffer, 0, size);
    } else {
        for (uint32_t i = 0; i < size; i++) {
            buffer[i] = micRing[micRd];
            micRd = (micRd + 1) % AUDIO_RING_BUF_SZ;
        }
    }
}

// Speaker OUT transfer callback: called when host sends speaker data
usb_status_t USB_AudioSpeakerIsoOut(usb_device_handle deviceHandle,
                                    usb_device_endpoint_callback_message_struct_t *event,
                                    void *arg)
{
    if ((event != NULL) && (event->buffer != NULL) && (event->length > 0))
    {
        for (uint32_t i = 0; i < event->length; i++)
        {
            spkRing[spkWr] = ((uint8_t*)event->buffer)[i];
            spkWr = (spkWr + 1) % AUDIO_RING_BUF_SZ;
            if (spkWr == spkRd)
                spkRd = (spkRd + 1) % AUDIO_RING_BUF_SZ; // prevent overflow
        }
    }
    // Prepare to receive next packet using static spkUsbBuf and AUDIO_PKT_SZ
    return USB_DeviceRecvRequest(deviceHandle, USB_AUDIO_STREAM_OUT_EP, spkUsbBuf, AUDIO_PKT_SZ);
}

// Microphone IN endpoint callback: queues mic data to host
usb_status_t USB_DeviceAudioIsoIn(usb_device_handle deviceHandle,
                                  usb_device_endpoint_callback_message_struct_t *event,
                                  void *arg)
{
    // Prepare next mic packet
    USB_AudioRecorderGetBuffer(s_wavBuff, AUDIO_PKT_SZ);
    // Send to host
    return USB_DeviceSendRequest(deviceHandle, USB_AUDIO_STREAM_ENDPOINT, s_wavBuff, AUDIO_PKT_SZ);
}
        ...
        if (0U != s_audioGenerator.attach)
        {
            ...
            if (s_audioGenerator.currentStreamInterfaceAlternateSetting != alternateSetting)
            {
                ...
                else
                {
                    ...
                    USB_AudioRecorderGetBuffer(s_wavBuff, (USB_SPEED_HIGH == s_audioGenerator.speed) ? HS_ISO_IN_ENDP_PACKET_SIZE : FS_ISO_IN_ENDP_PACKET_SIZE);
...
void APPInit(void)
{
    ...
    USB_DeviceIsrEnable();
    // --- build & register SPEAKER‐OUT isochronous endpoint ---
    static usb_device_endpoint_callback_struct_t spkCallback = {
        .callbackFn    = USB_AudioSpeakerIsoOut,
        .callbackParam = &s_audioGenerator
    };
    usb_device_endpoint_init_struct_t epOut = {
        .zlt            = 0,
        .transferType   = USB_ENDPOINT_ISOCHRONOUS,
        .endpointAddress= USB_AUDIO_STREAM_OUT_EP,
        .maxPacketSize  = AUDIO_PKT_SZ,
        .interval       = FS_ISO_OUT_ENDP_INTERVAL
    };
    USB_DeviceInitEndpoint(s_audioGenerator.deviceHandle, &epOut, &spkCallback);
    USB_DeviceRecvRequest(s_audioGenerator.deviceHandle, USB_AUDIO_STREAM_OUT_EP, spkUsbBuf, AUDIO_PKT_SZ);
    // --- build & register MICROPHONE-IN isochronous endpoint ---
    static usb_device_endpoint_callback_struct_t micCallback = {
        .callbackFn    = USB_DeviceAudioIsoIn,
        .callbackParam = &s_audioGenerator
    };
    usb_device_endpoint_init_struct_t epIn = {
        .zlt            = 0,
        .transferType   = USB_ENDPOINT_ISOCHRONOUS,
        .endpointAddress= USB_AUDIO_STREAM_ENDPOINT,
        .maxPacketSize  = AUDIO_PKT_SZ,
        .interval       = FS_ISO_IN_ENDP_INTERVAL
    };
    USB_DeviceInitEndpoint(s_audioGenerator.deviceHandle, &epIn, &micCallback);
    USB_DeviceSendRequest(s_audioGenerator.deviceHandle, USB_AUDIO_STREAM_ENDPOINT, s_wavBuff, AUDIO_PKT_SZ);
