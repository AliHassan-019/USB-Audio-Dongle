...
/*! @brief Whether USB Audio use sync mode or not (some SoCs don’t support it) */
...
/*-----------------------------------------------------------------------------
 * AUDIO FORMAT DEFINITIONS  (needed by usb_device_descriptor.c)
 *---------------------------------------------------------------------------*/
/* Sampling rate in kHz */
#if defined(AUDIO_DATA_SOURCE_DMIC) && (AUDIO_DATA_SOURCE_DMIC > 0U)
#define AUDIO_SAMPLING_RATE_KHZ (16U)
#else
#define AUDIO_SAMPLING_RATE_KHZ (8U)
#endif
...
/*-----------------------------------------------------------------------------
 * END AUDIO FORMAT DEFINITIONS
 *---------------------------------------------------------------------------*/

/* The one configuration we support is #1 */
#ifndef USB_AUDIO_GENERATOR_CONFIGURE_INDEX
#define USB_AUDIO_GENERATOR_CONFIGURE_INDEX (1U)
#endif

/* The clock-source entity that drives our mic */
#ifndef USB_AUDIO_RECORDER_CONTROL_CLOCK_SOURCE_ENTITY_ID
...
#define USB_AUDIO_STREAM_INTERFACE_INDEX         (1U)  /* mic IN */
#define USB_AUDIO_SPEAKER_STREAM_INTERFACE_INDEX (2U)  /* speaker OUT */

/* Alternate settings on those interfaces */
...
/* Total # of interfaces (1 control + 2 streaming) */
...
/* Interrupt packet sizes & intervals */
#define HS_INTERRUPT_IN_PACKET_SIZE (8U)
#define FS_INTERRUPT_IN_PACKET_SIZE (8U)
#define HS_INTERRUPT_IN_INTERVAL    (0x07U) /* 8 ms */
#define FS_INTERRUPT_IN_INTERVAL    (0x08U)

/* Isochronous IN (mic) packet sizes & intervals */
...
/* Isochronous OUT (speaker) packet sizes & interval */
 ...
 ******************************************************************************/
/*!
 * @brief USB device callback for handling class/config requests.
 */
