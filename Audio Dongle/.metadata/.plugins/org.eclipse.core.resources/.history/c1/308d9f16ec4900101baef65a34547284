...
#include "usb_device_descriptor.h"  /* for USB_AUDIO_STREAM_ENDPOINT, USB_AUDIO_OUT_ENDPOINT, etc. */
...
Audio PLL constants
      ...
      so AUDIO_PLL_USB1_SOF_INTERVAL_COUNT = (24576000 * 100 (counter interval)) /4000 = 614400
      AUDIO_PLL_FRACTIONAL_CHANGE_STEP
      The Audio input clock is 24Mhz, and denominator is 4500, divider is 15 and PFD is 26.
      so AUDIO_PLL_FRACTIONAL_CHANGE_STEP = (24000000 * 100 * 18) / (27000 * 26 * 15 *4000) + 1
**********************************************************************/
#define AUDIO_PLL_USB1_SOF_INTERVAL_COUNT (614400)  /* The USB1_SOF_TOGGLE's frequency is 4kHz. */
#define AUDIO_PLL_USB1_SOF_INTERVAL_COUNT1 (491520) /* 4kHz with alternate PLL */
#define AUDIO_PLL_FRACTIONAL_CHANGE_STEP    (2)
#endif

#define USB_DEVICE_INTERRUPT_PRIORITY (3U)
#define DATA_BUFF_SIZE               (AUDIO_SAMPLING_RATE_KHZ * AUDIO_FORMAT_CHANNELS * AUDIO_FORMAT_SIZE)

/*******************************************************************************
 * Prototypes
 ******************************************************************************/
/*!
 * @brief USB audio ISO OUT endpoint callback (microphone IN).
 */
...
/*!
 * @brief USB audio ISO OUT endpoint callback (speaker OUT).
 */
...
/*!
 * @brief Fill the microphone buffer (to send IN to host).
 */
extern void USB_AudioRecorderGetBuffer(uint8_t *buffer, uint32_t size);

/*!
 * @brief Consume the speaker buffer (received OUT from host) into the DAC.
 */
 ...
 ******************************************************************************/
/*!
 * @brief Audio generator (mic+speaker) state structure.
 */
typedef struct _usb_audio_generator_struct
{
    ...
    uint8_t copyProtect;
    uint8_t curMute;
    uint8_t curVolume[2]; /* need to consider endianness */
    uint8_t minVolume[2];
    uint8_t maxVolume[2];
    uint8_t resVolume[2];
    uint8_t curBass;
